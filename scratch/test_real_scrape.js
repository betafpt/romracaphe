...
// Xác định file session cookie có sẵn
...
async function testRealScrape() {
    console.log('🚀 Bắt đầu chạy thử nghiệm cào đơn hàng thực tế...');
    ...
    const browser = await chromium.launch({
        headless: true, // Chạy ẩn để thực thi nhanh chóng trên VPS
    ...
    try {
        ...
        console.log('✅ Kết nối thành công. Đang quét danh sách đơn hàng hiển thị trên màn hình...');
        
        // Tìm các card đơn hàng đang hiển thị
        const orderCards = page.locator('text="Đã làm xong"').locator('..').locator('..');
        const count = await orderCards.count().catch(() => 0);

        if (count === 0) {
            console.log('⚠️ Không tìm thấy đơn hàng nào ở trạng thái "Đã làm xong" trên màn hình lúc này.');
            console.log('Đang thử tìm kiếm bất kỳ đơn hàng nào có định dạng mã đơn...');
            const alternativeCards = page.locator('text=/^[A-Z0-9]+-[A-Z0-9]+$/').locator('..').locator('..');
            const altCount = await alternativeCards.count().catch(() => 0);
            if (altCount === 0) {
                console.log('❌ Không phát hiện bất kỳ đơn hàng nào trên màn hình portal để cào thử.');
                await browser.close();
                return;
            }
            console.log(`Phát hiện ${altCount} đơn hàng tiềm năng. Tiến hành cào đơn đầu tiên...`);
            await alternativeCards.first().click().catch(() => {});
        } else {
            console.log(`Phát hiện ${count} đơn hàng trên màn hình. Bắt đầu cào thử đơn đầu tiên...`);
            await orderCards.first().click().catch(() => {});
        }
        ...
        console.log('\n========================================================');
        console.log('🎉 KẾT QUẢ CÀO ĐƠN HÀNG THỰC TẾ THÀNH CÔNG!');
        console.log(JSON.stringify(result, null, 4));
        console.log('========================================================\n');

    } catch (e) {
        console.error('❌ Lỗi xảy ra trong quá trình cào thử đơn thực tế:', e.message);
    } finally {
        await browser.close();
